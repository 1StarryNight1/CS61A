...
def balanced(m):
    ...
    if not (balanced(end(left_arm)) and balanced(end(right_arm))):
        return False
    if total_mass(end(left_arm)) * left_len == total_mass(end(right_arm)) * right_len:
        return True
    return False
...
def max_path_sum(t):
    ...
    for branch in branches(t):
        if max_path_sum(branch) > m:
            m = max_path_sum(branch)
    return label(t) + m
