...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not location.is_hive:
            i += 1
            if not (i >= self.lower_bound and i <= self.upper_bound):
    ...
    def action(self, gamestate):
        ...
        while location.exit != None:
            ...
            if location.ant == None:
                continue
            if not location.ant.is_doubled:
                location.ant.double()
            if location.ant.is_container:
                if location.ant.ant_contained and not location.ant.ant_contained.is_doubled:
