...
def about(subject):
    ...
    def contain(paragraph):
        removed = remove_punctuation(paragraph)
        lower_paragraph = lower(removed)
        ls = split(lower_paragraph)
        for word in subject:
            if word in ls:
...
def accuracy(typed, source):
    ...
    if len_typed == 0 and len_source == 0:
        return 100.0
    if not (len_typed != 0 and len_source != 0):
        return 0.0
    count = 0
    i = 0
    while i < min(len_typed, len_source):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff <= limit and diff < minimum:
            minimum = diff
            closest = word
    if minimum > limit:
        return typed_word
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i)<time(match, minimum, i):
                minimum = j
        fastest[minimum].append(get_word(match, i))
    return fastest
    # END PROBLEM 10
